...
<script type="text/javascript" charset="utf-8" src="/dailychan/resources/js/oasis_common.js?dummy=5.04"></script>
	...
	<script type="text/javascript">
		...
		//팝업 관련 메소드
	     function openProductQnaForm(productQuestion_id) {
			
	    	 var url = "/dailychan/admin/productQna/productQuestionData.action";
	    		
	    		$.post(url,{productQuestion_id:productQuestion_id},function(data){
	     ...
	     function productAnswer() {
	    	...
	    	var productQuestion_id = $('[name="productQuestion_id"]').val();
	    	var productAnswer_content = $('[name="productAnswer_content"]').val();
	    	
	    	var url = "/dailychan/admin/productQna/productAnswer.action";
	    	
	    	$.post(url,{join_id:join_id,product_id:product_id,productQuestion_id:productQuestion_id,
	    		productAnswer_content:productAnswer_content},function(data){
										...
										<c:if test="${productQnaDataCount!=0 }">
											...
											<tr>
												...
												<td class="tL"><a href="#" onclick="return false;">
														${dto.product_subject } </a></td>
												<td>${dto.join_id }</td>
												<td>${dto.productQuestion_created }</td>
												<td><input type="button" onclick="openProductQnaForm('${dto.productQuestion_id}'); return false;" class="btn green" value="답변"/></td>
