...
<script type="text/javascript" charset="utf-8" src="/dailychan/resources/js/oasis_common.js?dummy=5.12"></script>
	...
	<script type="text/javascript">
		...
		function order() {
			...
			var selectedProductKeies = [];
			var selectedProductPriceKeies = [];
			var selectedProductCount = 0;
			
			$(".chk_product:checked").each(function() {
				selectedProductKeies.push($(this).parents("tr").data("product_id"));
				selectedProductPriceKeies.push($(this).val());
				selectedProductCount++;
			});
			
			alert(selectedProductCount);
			
			if (selectedProductKeies.length == 0) {
				alert("상품을 선택해 주세요.");
			} else {

	        	location.href = "/dailychan/myPage/nowBuy.action?selectedProductCount=" + selectedProductCount + "&selectedProductKeies=" + selectedProductKeies +
				"&selectedProductPriceKeies=" + selectedProductPriceKeies;
						...
						<div class="mypage-contents">
							<div class="mypage-cblock">
								...
								<div class="mypage-order-info">
									<div class="row">
										<ul class="dash">
											<li>자주구매하는 상품 중 최대 50가지를 선정하여 보여집니다.</li>
											<li>구매시점의 상품가격 및 이벤트가 변경될 수 있으며 조기품절 될 수 있습니다.</li>
										</ul>
									</div>
								</div>
									...
									<table>
										...
										<tbody>
											...
											<c:if test="${dataCount==0 }">
												...
												<tr>
													<td colspan="6">
						                                <div class="empty-msg">구매 내역이 없습니다.</div>
													</td>
												</tr>
											...
											<c:if test="${dataCount!=0 }">
												<c:forEach var="dto" items="${lists }">
													<tr data-product_id='${dto.product_id }'>
														<th>
															<label class="checkbox">
															<input type="checkbox" id="checkbox" value="${dto.product_resultPrice }" class="chk_product">
															<i></i>
														</label>
														</th>
														<th>
								...
								<div class="btn-wrap">
									<a href="#;" class="btn gray lg" onclick="order(); return false;">선택상품 재구매</a>
									<a href="#;" class="btn lred lg" onclick="checkAllBtn(); order(); return false;">전체상품 재구매</a>
								</div>
							</div>
						</div>
					</div>
